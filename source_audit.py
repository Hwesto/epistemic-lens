...
Outputs six sections:
  ...
  6. Story-relevant editorial gaps (hardcoded narrative-specific notes)

Developer tool. Not invoked by any workflow. Run directly:

    python source_audit.py

All audit logic is gated behind `if __name__ == "__main__": main()` so that
`import source_audit` is side-effect-free — no HTTP, no stdout, no file IO.
"""
...
# Region taxonomy. Each `present` entry must be a country bucket key in
# feeds.json. main() runs a sanity-check loop that warns on stale entries
# (e.g. a removed bucket that's still listed here as present) — Gap 16-4.
REGIONS = {
...
def probe(name_url):
    """Live HTTP probe of one feed URL. Returns dict of observed facts."""
    name, url = name_url
...
def main() -> int:
    """Run the full source audit. Prints + writes source_audit.json.

    Wrapped in a function so `import source_audit` is side-effect-free —
    nothing fires HTTP, nothing prints, nothing writes to disk until the
    caller invokes main() (or the `if __name__ == "__main__"` guard
    triggers from direct invocation).
    """
    ...
    # Gap 16-4: sanity-check that REGIONS doesn't list stale `present`
    # entries — i.e. country keys that no longer appear in feeds.json.
    # Stripped values are entries like "wire_services (FR via AFP)" which
    # are editorial annotations; only the bare-key form is checked.
    stale = []
    for region, data in REGIONS.items():
        for entry in data.get("present", []):
            key = entry.split(" ", 1)[0]
            if key.isidentifier() and key not in PRESENT:
                stale.append((region, key))
    if stale:
        print("\n  WARN: REGIONS lists buckets that aren't in feeds.json:")
        for region, key in stale:
            print(f"    - {region}: '{key}' (no longer a country key)")
