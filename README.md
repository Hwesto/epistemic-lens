...
| **meta-v7.3.0** | **May 2026** | **Current. Phase 3a-3c: source/quote attribution. Per-quote speaker + role + type + attributive verb + stance toward target with verbatim quote-grounding. Per-outlet / per-bucket / per-region aggregation; "Voices" sections in renderer + web frontend; `api/sources/<DATE>/<story>/sources.json` and `api/sources/aggregate/<DATE>.json`.** |
